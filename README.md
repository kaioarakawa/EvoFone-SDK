...
```bash
composer require laravel/foneEvo-sdk
```
...
```php
$forge = new EvoFone\FoneEvo(TOKEN_HERE);
```
...
This will give you an array of servers that you have access to, where each server is represented by an instance of `EvoFone\Resources\Server`, this instance has multiple public properties like `$name`, `$id`, `$size`, `$region`, and others.
...
This method will ping Forge servers every 5 seconds and see if the newly created Site's status is `installed` and only return when it's so, in case the waiting exceeded 30 seconds a `EvoFone\Exceptions\TimeoutException` will be thrown.
